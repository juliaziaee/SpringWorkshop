...
#there are different types of geoms you can use to do different things to the plot
```
...
#stores new vector called sum_xyz
...
#pipes diamonds through function mutate; adds new variable "total" to dataset
diamonds <- diamonds %>%
  mutate(total = x + y + z)
```
...
#adds another variable column to the dataset
```
...
#filters data so you can only look at smth that fits those qualifications; makes new data subset
